...
from model.ate_bio import ATEModel as Model
from utils.ate_bio_datamodule import DataModule
from utils.ate_result import Result
...
class LightningModule(pl.LightningModule):
    ...
    def training_epoch_end(self, outputs):
        examples = self.data_module.raw_datasets['train']

        self.current_train_result = Result.parse_from(outputs, examples)
        self.current_train_result.cal_metric()

        if not hasattr(self, 'best_train_result'):
            self.best_train_result = self.current_train_result
    
        elif self.best_train_result < self.current_train_result:
            self.best_train_result = self.current_train_result
